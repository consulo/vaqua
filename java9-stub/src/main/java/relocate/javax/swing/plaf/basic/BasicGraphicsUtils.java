 ...
 * questions.
 */
package relocate.javax.swing.plaf.basic;
...
public class BasicGraphicsUtils {
    ...
    public static void drawStringUnderlineCharAt(JComponent c, Graphics2D g, String string, int underlinedIndex, float x, float y) {
        try {
            Class<?> clazz = Class.forName(BasicGraphicsUtils.class.getName().replace("relocate.", ""));

            Method method = clazz.getDeclaredMethod("drawStringUnderlineCharAt", JComponent.class, Graphics2D.class, String.class, int.class, float.class, float.class);
            method.setAccessible(true);

            method.invoke(null, c, g, string, underlinedIndex, x, y);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
